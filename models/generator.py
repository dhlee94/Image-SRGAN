...
class Generator(nn.Module):
    def __init__(self, img_size=128, channels=1, out_channels=1, filtersize=[64, 128, 256, 512], up_mode="bilinear", check_sigmoid=False):
        ...
        #####################################################################################################################
        self.final_upsample = self.Up = nn.Sequential(
                LayerNorm(self.filtersize[0], eps=1e-6,  data_format="channels_first"),
                nn.ConvTranspose2d(self.filtersize[0], self.filtersize[0], kernel_size=4, stride=4, padding=0, bias=False)
            )
